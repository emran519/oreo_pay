...
if($verify_result) {//验证成功
  ...
  $trade_status = $_GET['trade_status'];
    ...
    //付款完成后，系统发送该交易状态通知
    $srow=$DB->query("SELECT * FROM opay_regcode WHERE trade_no='{$trade_no}' limit 1")->fetch();
    ...
    if($srow['status']==0){
    $DB->exec("update `opay_regcode` set `status` ='1' where `id`='{$srow['id']}'");
    $key = random(32);
    $sds=$DB->exec("INSERT INTO `oreo_user` (`key`, `account`, `username`, `money`, `url`, `email`, `phone`, `addtime`, `type`, `active`) VALUES ('{$key}', '{$account}', '{$username}', '0', '{$url}', '{$email}', '{$phone}', '{$date}', '1', '1')");
    $pid=$DB->lastInsertId();
      if($sds){
        if(!empty($tid)){
          $sj=$conf['price'];
          $abc=$DB->exec("UPDATE `oreo_user` SET `money`=money+'{$sj}',`price`=price+'{$sj}' WHERE `id`='$tid'");
          $abc1=$DB->exec("UPDATE `oreo_user` SET `tgrs`=tgrs+'1' WHERE `id`='$tid'");
        }
