...
if($verify_result) {//验证成功
    ...
    if ($_GET['trade_status'] == 'TRADE_SUCCESS' || $_GET['trade_status'] == 'TRADE_FINISHED') {
      ...
      if($srow['status']==0 && $conf['sw_money_rate']==0 && $user['zdyfl']==0){
			...
			$sqs=$DB->exec("update `oreo_lxjk` set `oreo_lxknum` ='{$oreo_lxknum}' where oreo_lxname='2' AND oreo_lxtype='1' AND oreo_lxurl='$lxurl'");
			...
			$sqs=$DB->exec("update `oreo_lxjk` set `oreo_lxknum` ='{$oreo_lxknum}', oreo_lrje=oreo_lrje+{$addmoney} where oreo_lxname='2' AND oreo_lxtype='1' AND oreo_lxurl='$lxurl'");
			}
