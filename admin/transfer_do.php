...
$appid = $conf['alipay_appid'];  //https://open.alipay.com 账户中心->密钥管理->开放平台密钥，填写添加了电脑网站支付的应用的APPID
$outTradeNo = uniqid();     //商户转账唯一订单号
$payAmount = $row['money'];           //转账金额，单位：元 （金额必须大于等于0.1元)
$remark = $row['payer_show_name'];    //转帐备注
$signType = 'RSA2';       //签名算法类型，支持RSA2和RSA，推荐使用RSA2
//商户私钥，填写对应签名算法类型的私钥，如何生成密钥参考：https://docs.open.alipay.com/291/105971和https://docs.open.alipay.com/200/105310
$saPrivateKey=$_SESSION['privatekey'];
$account = $row['account'];      //收款方账户（支付宝登录号，支持邮箱和手机号格式。）
$realName = $row['username'];     //收款方真实姓名
...
$DB->exec("update `oreo_settle` set `transfer_status`='1',`transfer_result`='".$result['order_id']."',`transfer_date`='".$result['pay_date']."' where `id`='$id'");  
}elseif($result['code']=='40004') {
$data['code']=0;
$data['ret']=0;
$data['msg']='fail';
$data['result']='失败 ['.$result['sub_code'].']'.$result['sub_msg'];
$DB->exec("update `oreo_settle` set `transfer_status`='2',`transfer_result`='".$data['result']."' where `id`='$id'");
}
else{
