...
if(isset($_POST['user']) && isset($_POST['pass'])){
    $admin_user=daddslashes($_POST['user']);
    $admin_pass=daddslashes($_POST['pass']);
    $safepassword=daddslashes($_POST['safepassword']);
    if($admin_user==''||$admin_pass=='') {
        exit("<script language='javascript'>alert('请不要提交空参数');history.go(-1);</script>");
    }
    if($safe!==$safepassword){
        echo "<script>alert('安全码错误！');window.location.href='index.php'</script>";
        setcookie("admin_token", "", time() - 259200);
        session_unset();
        session_destroy();
        exit();
    }else {
        $pass = md5($_POST['pass'].$password_hash);
        if($conf['oreo_admin']==$admin_user&&$conf['oreo_password']==$pass){
            $session=md5($admin_user.$pass.$password_hash);
            $expiretime=time()+604800;
            $token=authcode("{$admin_user}\t{$session}\t{$expiretime}", 'ENCODE', SYS_KEY);
            setcookie("admin_token", $token, time() + 604800);
            $city=get_ip_city($clientip);
            $DB->query("insert into `oreo_log` (`uid`,`type`,`date`,`city`,`data`) values ('1','登录Oreo支付系统控制台','".$date."','".$city."','".$clientip."')");
            @header('Content-Type: text/html; charset=UTF-8');
            exit("<script language='javascript'>alert('登录成功！');window.location.href='./';</script>");
        }else{
            exit("<script language='javascript'>alert('账号或密码有误');window.location.href='./';</script>");
        }
...
<head>
    ...
    <title><?=$conf['web_name']?>登录界面</title>
    <link href="../assets/new_admin_login/css/default.css" rel="stylesheet" type="text/css" />
    <!--必要样式-->
    <link href="../assets/new_admin_login/css/styles.css" rel="stylesheet" type="text/css" />
    <link href="../assets/new_admin_login/css/demo.css" rel="stylesheet" type="text/css" />
    <link href="../assets/new_admin_login/css/loaders.css" rel="stylesheet" type="text/css" />
</head>
    ...
    <form action="login.php" method="post" class="form-horizontal" role="form">
        <div class='login_fields'>
            <div class='login_fields__user'>
                <div class='icon'>
                    <img alt="" src='../assets/new_admin_login/img/user_icon_copy.png'>
                </div>
                <input name="user" placeholder='用户名' maxlength="16" type='text' autocomplete="off"  />
                <div class='validation'>
                    <img alt="" src='../assets/new_admin_login/img/tick.png'>
                </div>
            </div>
            <div class='login_fields__password'>
                <div class='icon'>
                    <img alt="" src='../assets/new_admin_login/img/lock_icon_copy.png'>
                </div>
                <input name="pass" placeholder='密码' maxlength="16" type='text' autocomplete="off">
                <div class='validation'>
                    <img alt="" src='../assets/new_admin_login/img/tick.png'>
                </div>
            </div>
            <div class='login_fields__password'>
                <div class='icon'>
                    <img alt="" src='../assets/new_admin_login/img/key.png'>
                </div>
                <input name="safepassword" placeholder='安全码' maxlength="16" type='text' name="safepassword" autocomplete="off">
...
<link href="../assets/new_admin_login/layui/css/layui.css" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="../assets/new_admin_login/js/jquery.min.js"></script>
<script type="text/javascript" src="../assets/new_admin_login/js/jquery-ui.min.js"></script>
<script type="text/javascript" src='../assets/new_admin_login/js/stopExecutionOnTimeout.js?t=1'></script>
<script type="text/javascript" src="../assets/new_admin_login/layui/layui.js"></script>
<script type="text/javascript" src="../assets/new_admin_login/js/Particleground.js"></script>
<script type="text/javascript" src="../assets/new_admin_login/js/Treatment.js"></script>
<script type="text/javascript" src="../assets/new_admin_login/js/jquery.mockjax.js"></script>
    ...
    layui.use('layer', function() {
        ...
        $('input[type="button"]').click(function() {
            ...
            if(user == '') {
                ...
                ErroAlert('请输入密码');
            } else if(safepassword == '' || safepassword.length != 4) {
                ErroAlert('输入安全码');
            }
        })
    })
...
<script type="text/javascript" src="../assets/new_admin_login/img/ThreeWebGL.js"></script>
<script type="text/javascript" src="../assets/new_admin_login/img/ThreeExtras.js"></script>
<script type="text/javascript" src="../assets/new_admin_login/img/Detector.js"></script>
<script type="text/javascript" src="../assets/new_admin_login/img/RequestAnimationFrame.js"></script>
